...
useSeoMeta({
  title: `NeoForge ${version.id} Join Guide · Minecraft Crowd Control`,
  description: `NeoForge ${version.id} Join Guide`,
  ogDescription: `NeoForge ${version.id} Join Guide`,
})
</script>

<template>
  <div>
    <h1>NeoForge {{ version.id }}: Joining an External Server</h1>
    ...
    <ol v-if="!version.legacy">
      ...
      <li>(Optional) For extra effects and the best experience, install the client mod using the <NuxtLink :to="`/guide/neoforge/client?v=${version.id}`">Client Setup</NuxtLink> guide.</li>
    ...
    <ol v-else>
      ...
      <li>Select <strong>Start Session</strong> in the Crowd Control app.
        <ul>
