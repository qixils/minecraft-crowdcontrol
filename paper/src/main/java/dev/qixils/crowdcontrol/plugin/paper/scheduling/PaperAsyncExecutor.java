...
public record PaperAsyncExecutor(PaperCrowdControlPlugin plugin) implements AgnosticExecutor {
	...
	public @NotNull AgnosticTask runRepeating(long delay, long period, @NotNull Consumer<AgnosticTask> consumer) {
		...
		AgnosticTask task = new PaperTask(scheduler.runTaskTimerAsynchronously(plugin, (Object) bukkitConsumer, delay, period));
		taskRef.set(task);
		return task;
	}
