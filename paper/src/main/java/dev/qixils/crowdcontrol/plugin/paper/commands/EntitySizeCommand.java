				...
				for (Entity entity : world.getEntities()) {
					...
					for (Location loc : locations) {
						...
						if (AttributeUtil.getModifier(living, Attribute.GENERIC_SCALE, SCALE_MODIFIER_UUID).map(AttributeModifier::getAmount).orElse(0d) == level)
							continue;
						entity.getScheduler().run(plugin.getPaperPlugin(), $ -> addModifier(living, Attribute.GENERIC_SCALE, SCALE_MODIFIER_UUID, SCALE_MODIFIER_NAME, level, AttributeModifier.Operation.ADD_SCALAR, true), null);
						success = true;
						break;
					}
				}
