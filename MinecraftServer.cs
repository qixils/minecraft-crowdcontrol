...
namespace CrowdControl.Games.Packs
{
    ...
    public class MinecraftServer : SimpleTCPPack<SimpleTCPClientConnector>
    {
        ...
        private static readonly List<Effect> AllEffects = new()
        {
            ...
            new Effect("Low Gravity", "low_gravity", "gravity") { Price = 50, Description = "Decreases the streamer's gravity" },
            new Effect("Maximum Gravity", "maximum_gravity", "gravity") { Price = 200, Description = "Maximizes the streamer's gravity" },
            new Effect("Zero Gravity", "zero_gravity", "gravity") { Price = 75, Description = "Disables the streamer's gravity" },
            // freeze-type commands
            new Effect("Freezing", "freezing", ItemKind.Folder),
            new Effect("Freeze Feet (10s)", "freeze", "freezing") { Price = 100, Description = "Temporarily prohibits movement" },
            new Effect("Freeze Camera (10s)", "camera_lock", "freezing") { Price = 100, Description = "Temporarily freeze the streamer's camera" },
            new Effect("Lock Camera To Ground (10s)", "camera_lock_to_ground", "freezing") { Price = 150, Description = "Temporarily locks the streamer's camera to the ground" },
            new Effect("Lock Camera To Sky (10s)", "camera_lock_to_sky", "freezing") { Price = 150, Description = "Temporarily locks the streamer's camera to the sky" },
            new Effect("Disable Jumping (10s)", "disable_jumping", "freezing") { Price = 50, Description = "Temporarily prevents the streamer from jumping" },
            // inventory commands
