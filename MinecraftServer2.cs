...
namespace CrowdControl.Games.Packs
{
    ...
    public class MinecraftServer : SimpleTCPPack<SimpleTCPClientConnector>
    {
        ...
        private static readonly EffectList AllEffects = new(new Effect[]
        {
            ...
            new("Plant Tree", "plant_tree") { Price = 100, Description = "Plant a tree on top of every player" },
            ...
            new("Respawn Player", "respawn") { Price = 500, Description = "Sends players to their spawn point" },
            new("Spawn Ore Veins", "vein") { Price = 100, Description = "Places random ore veins (or lava) near every player" },
            ...
            new("Teleport All Entities To Players", "entity_chaos") { Price = 3000, Description = "Teleports every loaded mob on the server to the targeted players in an even split. Note that this may only teleport nearby mobs on certain server configurations." },
            new("Water Bucket Clutch", "bucket_clutch") { Price = 400, Description = "Teleports players 30 blocks up and gives them a water bucket, forcing them to clutch if they want to live" },
            // time commands
            ...
            // health commands
            new("Take Max Health", "max_health_sub") { Price = 100, Quantity = 10, Description = "Subtracts half hearts from every player's max health" },
            new("Give Max Health", "max_health_add") { Price = 50, Quantity = 10, Description = "Adds half hearts to every player's max health" },
            new("Damage Player", "damage") { Price = 25, Quantity = 10, Description = "Damages players for the set number of hearts, unless it would kill them" },
            new("Heal Player", "heal") { Price = 10, Quantity = 10, Description = "Heals players for the set number of hearts" },
            new("Heal Player to Full", "full_heal") { Price = 50, Description = "Resets every player's health to full" },
            new("Halve Health", "half_health") { Price = 125, Description = "Sets every player's health to 50% of what they currently have" },
            new("Kill Player", "kill") { Price = 1000, Description = "Immediately kills every player on the spot" },
            new("Invincible", "invincible") { Price = 50, Description = "Temporarily makes players immune to damage", Duration = 15 },
            new("One-Hit KO", "ohko") { Price = 250, Description = "Temporarily makes any damage source kill players in one hit", Duration = 15 },
            // food commands
            ...
            new("Reset Experience", "reset_exp_progress") { Price = 500, Description = "Clears every players' XP" },
            // gravity commands
            new("High Gravity", "high_gravity") { Price = 100, Duration = 20, Description = "Increases player gravity, allowing players to jump higher and fall slower" },
            new("Low Gravity", "low_gravity") { Price = 50, Duration = 20, Description = "Decreases player gravity, disallowing players from jumping up blocks" },
            new("Maximum Gravity", "maximum_gravity") { Price = 200, Duration = 20, Description = "Maximizes player gravity, disallowing players from jumping up blocks and rapidly pulling them down to the ground" },
            new("Zero Gravity", "zero_gravity") { Price = 100, Duration = 20, Description = "Disables player gravity, allowing players to float in the air" },
            // freeze-type commands
            new("Can't Move", "freeze") { Price = 100, Duration = 15, Description = "Temporarily prohibits player movement" },
            new("Lock Camera", "camera_lock") { Price = 100, Duration = 15, Description = "Temporarily freezes every player's cameras" },
            new("Lock Camera To Ground", "camera_lock_to_ground") { Price = 200, Duration = 15, Description = "Temporarily locks every player's cameras to the ground" },
            new("Lock Camera To Sky", "camera_lock_to_sky") { Price = 200, Duration = 15, Description = "Temporarily locks every player's camera to the sky" },
            new("Disable Jumping", "disable_jumping") { Price = 100, Duration = 15, Description = "Temporarily prevents players from jumping" },
            // inventory commands
            new("Clear Inventory", "clear_inventory") { Price = 750, Description = "Wipes every item from players' inventories. Unavailable when 'Enable Keep Inventory' is active." },
            new("Clutter Inventory", "clutter") { Price = 50, Description = "Shuffles around items in every player's inventory" },
            new("Damage Item", "damage_item") { Price = 100, Description = "Takes 25% of the durability of a held or worn item" },
            new("Delete Held Item", "delete_item") { Price = 200, Description = "Deletes whatever item every player is currently holding" },
            new("Delete Random Item", "delete_random_item") { Price = 150, Description = "Deletes a random item from every player's inventory" },
            new("Disable Keep Inventory", "keep_inventory_off") { Price = 300, Description = "Disallows players from keeping their inventory upon death" }, // TODO: bid war?
            new("Drop Held Item", "drop_item") { Price = 25, Description = "Makes players drop their held item" },
            new("Enable Keep Inventory", "keep_inventory_on") { Price = 100, Description = "Allows players to keep their inventory upon death" },
            new("Put Held Item on Head", "hat") { Price = 50, Description = "Swaps the item in every player's hand slot and head slot" },
            new("Repair Item", "repair_item") { Price = 50, Description = "Fully repairs a damaged item" },
            // set gamemode for x seconds
            new("Adventure Mode", "adventure_mode") { Price = 200, Duration = 20, Description = "Temporarily sets players to Adventure mode, rendering them unable to place or break blocks" },
            new("Creative Mode", "creative_mode") { Price = 200, Duration = 20, Description = "Temporarily sets players to Creative mode, allowing them to fly and spawn in items" },
            new("Spectator Mode", "spectator_mode") { Price = 200, Duration = 10, Description = "Temporarily sets players to Spectator mode, turning them into a ghost that can fly through blocks" },
            new("Allow Flight", "flight") { Price = 150, Duration = 20, Description = "Temporarily allows players to fly" },
            ...
            new("Bedrock", "block_bedrock") { Price = 200, Description = "Places the unbreakable bedrock block at every player's feet" },
            new("Cobweb", "block_cobweb") { Price = 25, Description = "Places a cobweb block at every player's feet to slow them down" },
            // TODO: temporarily disabled as it's prone to failure. should add failsafes or replace with a new effect that uses API methods directly. | new("Fire", "block_fire") { Price = 200, Description = "Places a fire block at every player's feet to set them ablaze" },
            new("Sculk Catalyst", "block_sculk_catalyst") { Price = 25, Description = "Places a sculk catalyst at every player's feet to absorb the XP from mobs that die nearby" },
            new("Wither Rose", "block_wither_rose") { Price = 100, Description = "Places a toxic wither rose at every player's feet to poison them" },
            new("Lightning Rod", "block_lightning_rod") { Price = 25, Description = "Places a lightning rod at every player's feet to attract lightning strikes" },
            new("Water", "block_water") { Price = 50, Description = "Places a flowing water block at every player's feet" },
            // places a block several blocks above everyone's head
            new("Falling Anvil", "falling_block_anvil") { Price = 100, Description = "Drops a genuine ACME corp. anvil block on every player" },
            new("Falling Sand", "falling_block_sand") { Price = 25, Description = "Drops a sand block on every player" }, // TODO: drop random between Sand, Red Sand, and Gravel. and maybe drop a whole column?
            // TODO: new("Random Falling Block", "falling_block_random") { Price = 50, Description = "Drops a totally random block on every player" },
